...
customer_classes <- c("RESIDENTIAL_SINGLE", "RESIDENTIAL_MULTI", "COMMERCIAL")
#End
...
for(i in 1:(numOfFiles*length(customer_classes)))
{
  #Determine the customer class to test
  df_class <- df_sample %>% filter(cust_class == customer_classes[ceiling(i/numOfFiles)])
  #End
  ...
  tryCatch(
    ...
    error = function(cond)
    {
      ...
      message(paste("Error in file:", file_names[index], "\n", cond, "\n"))
      #End
    },
    finally =
    {
...
df_final_bill <- df_final_bill %>% arrange(utility_id)
#End

#Output The Final Bill information to a CSV file
write.csv(df_final_bill, "../../Dropbox/CSV/sampleBill.csv")
#End
