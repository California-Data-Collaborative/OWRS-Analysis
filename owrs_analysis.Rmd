...
```{r mean_bill_by_parts_pie, echo=FALSE, fig.cap = mean_bill_pie_cap}
...
```{r rate_structure_type_pie, echo=FALSE}
plot_rate_type_pie(df_final_bill)


```

```{r commodity_charge_vs_usage_line, echo=FALSE}
...
```{r commodity_charge_vs_usage_boxplot, echo=FALSE}
...
```{r service_charge_ratio_histogram, echo=FALSE}
...
```{r total_bill_histogram, echo=FALSE}
...
```{r efficiency_goal_time_series_boxplot, echo=FALSE, warning=FALSE}
plot_efficiency_ts(merged_OWRS)

```

```{r gpcd_time_series_boxplot, echo=FALSE, warning=FALSE}
plot_gpcd_ts(merged_OWRS)
```
## Compare Rates and efficiency
```{r, echo=FALSE}
df_final_bill$utility_name_supplier_report <- as.character(sapply(df_final_bill$utility_name, GetCloseMatches,
                                                sequence_strings = supplier_pwsid$Agency_Name, n=1L, cutoff = 0.85))
...
manually_mapped_names <- as.character(owrs_to_supplier_report_manual_map[df_final_bill$utility_name])
df_final_bill$utility_name_supplier_report <- ifelse(manually_mapped_names == "NULL", 
                                               df_final_bill$utility_name_supplier_report,
                                               manually_mapped_names)



df_final_bill <- merge(df_final_bill, supplier_pwsid, by.x = "utility_name_supplier_report", by.y = "Agency_Name", all.x=TRUE, all.y=FALSE)
...
```{r efficiency_goal_vs_total_bill_scatter_trend, echo=FALSE, warning=FALSE}
plot_eff_vs_bill(eff_vs_rate, singleTargetValue)
```
...
```{r efficiency_goal_vs_percent_fixed_scatter_trend, echo=FALSE, warning=FALSE}
...
plot_eff_vs_pctFixed(eff_vs_rate %>% filter(usage_ccf == singleTargetValue))
```

```{r fixed_cost_percentage_histogram, echo=FALSE, warning=FALSE}
...
```{r fixed_revenue_percentage_histogram, echo=FALSE, warning=FALSE}
pct_rev <- ggplot(quali_survey, aes(rev_pct_fixed)) + geom_histogram(bins = 10)

pct_rev
```

```{r fixed_costs_vs_fixed_rev_scatter, echo=FALSE, warning=FALSE}
quali_survey$fixedRev_per_fixedCosts <- quali_survey$rev_pct_fixed / quali_survey$costs_pct_fixed

ggplot(quali_survey, aes(x=costs_pct_fixed, y=rev_pct_fixed)) + geom_point()
```
